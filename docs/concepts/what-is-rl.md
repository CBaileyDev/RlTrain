...
Picture one car on the field. Fifteen times a second, roughly, the following happens.
...
- **Timestep** — one pass around the loop. Timesteps are numbered: `t = 0` at kickoff, then 1, 2, 3. A little `t` written under a symbol, like `o_t`, means "the value of that thing at timestep t".
- **Agent** — the thing making decisions. In RL Studio the agent is a neural network. It takes numbers in and puts numbers out.
- **Policy** — the rule the agent uses to turn an observation into an action. In RL Studio the policy *is* the neural network: feed it the observation vector and it gives you a probability for each control input it could pick. When people say "the bot improved", they mean the policy changed. Written `π` in papers, and `policy(obs)` in the code below. [policy-and-value.md](./policy-and-value.md) is the page about it.
- **Environment** — everything the agent does not control. Here that is [RocketSim](https://github.com/ZealanL/RocketSim) simulating Soccar: the car physics, the ball, the walls, boost pads, the goal detection, and the opponent car.
...
- **Transition** — the record of one pass: observation, action, reward, next observation, done. Those five parts are the core of the record, and they are what the concept is. RL Studio actually stores a couple more fields alongside them — the probability the policy assigned to the action it picked, and the value network's estimate for that state — because the update step needs them; [training-loop.md](./training-loop.md) lists the full record.
...
Note the sampling. The policy does not output one action — it outputs a probability for every action, and the loop draws from that. That draw is where exploration comes from; see the exploration section below.
...
So a reward of `-0.3` does not mean "steer harder". It means "that step scored -0.3", and nothing more. On its own that number is not even good or bad — it sits on whatever scale you chose when you wrote the reward function. If the bot usually collects -1.0 per step, then -0.3 was a good step. A reward function where every single step returns a negative number trains perfectly well.
...
- **A goal is scored.** The match genuinely ended. There is no future beyond this point.
...
Supervised learning needs labelled examples: input, correct output, repeat. To train a Rocket League bot that way you would need, for each of a million game states, the correct control input for that state. Nobody can produce those labels. There is no answer key. Even a Grand Champion cannot sit down and write out the correct throttle-steer-jump combination for a million frames, and for most frames there are several good answers anyway.

Reinforcement learning sidesteps this. It never needs the correct action. It only needs a number saying how the chosen action turned out. That number is cheap to produce: RocketSim already knows where everything is and how fast it is moving, so a reward function you configure can compute a score for every step without any human labelling it. You still have to decide what that function rewards — that is real design work, and [rewards.md](./rewards.md) is about it — but you write it once and it then scores millions of steps unattended.

**Behaviour cloning is a real alternative, and it is worth being honest about.** You can take recorded replays of human matches, treat each frame's observation as the input and the human's actual control input as the label, and train with ordinary supervised learning. This works. It is often much faster than RL to get something that looks like Rocket League. What it gives you is an imitation of the demonstrator's *average* behaviour, capped at roughly the demonstrator's skill. What it does not give you is improvement past the data — a cloned bot has no mechanism for discovering a play the demonstrator never made. Serious projects sometimes clone first and then improve with RL. RL Studio is built around the RL half because that is the half this project is for learning, not because cloning is worse.
...
- **`env.tick_skip`** — how many 120 Hz physics ticks each decision is held for. The default of 8 gives 15 decisions per second. Turn it down for finer control at a proportionally higher cost per second of game time; turn it up for cheaper training and a clumsier bot. See [actions.md](./actions.md).
